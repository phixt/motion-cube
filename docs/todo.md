...
- 版本 **0.4.1**（package.json 与 src-tauri/tauri.conf.json 同步；2026-09-12 发布
  ...
  十七~十九轮 + 外部注入 API A 期与三件套文档；本地编译已交付，发布由用户执行）
...
### 0.4.1 发布准备（2026-09-12）

- ✅ **bump 0.4.1**（7f43162，双端同步；内容清单见「当前状态」）。
- ✅ **三文档信息同步**：README 新增「项目结构」速查树 + 首页简介补求解器/双手 +
  注入 API 双手语义补记；AGENTS.md 项目速览补手部/双手双轨/主位手规则/注入 API/
  验证套件；本文件版本状态与台账更新。
- ✅ **tauri 本地编译**（`npm run tauri build`，增量 41.7s）：产物
  `src-tauri/target/release/motion-cube.exe` + `bundle/nsis/MotionCube_0.4.1_x64-setup.exe`；
  启动验证 Start-Process + 截图（首页 Mica/伪 3D 魔方/视差渲染正常）。
  **不 push、不打 tag**——发布由用户执行（tag 触发 Release CI 上传 Releases）。
...
- ✅ **左右手同时显示**：（0.4.0 十七~十九轮已落地，见上手部 low-poly 节——
  left/right 双轨数据层 + 双实例渲染 + 主位手规则；旧档解析迁移平滑兼容）
...
- ⬜ **MCP 预览服务**：加一个 MCP 预览（截图/状态查询），方便 AI 写作时实时查看
  应用渲染与页面状态；与外部注入 API（A 期已实现+真机验证通过，B 期帧流暂缓）
  共用底层能力。
