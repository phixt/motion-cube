...
### 求解器 / 公式库
- ✅ **EOLR 一步表命中率 ~0 —— 实证闭环（research/solver-advanced R1，已提交）**：
  根因不是匹配 bug，是 **setup 数据覆盖不足**。实证（probe-eolr-rootcause.ts N=300：
  CMLL 后裸 LSE 态去查位置级指纹表 → 0/296 交集、命中 0%——位置级键必不匹配；
  probe-eolr-class-ratio.ts N=300：先把 4a（EO 定向）做完再统计——EO done 100% 自检，
  UL/UR 均在 U 槽（Arrow 完整情形）占 45.7%（EOLR 一步本应可服务的面不低），但
  46 条 setup 覆盖的「(UL,UR,DF,DB) 四棱槽组合」与真实 4a 后四棱排列 **0 交集**
  （四棱排列空间 ≈576，46 条只覆盖规范 Arrow/1-1 位）→ 即使改分类键也仅覆盖
  <1% 样本。**结论：EOLR 一步表路线当前数据不足以实质命中，维持现有效果**
  （4a 用 T.eo 搜索 + 6E2C，即 roux-adv）。后续真要做 EOLR：需爬全 4 类情形
  （Arrow/1-1/2-2/4-4 完整 setup 集）或改「先 Arrow 后 6E2C」分叉口（Arrow 前置
  45.7% 时先一步归 UL/UR 再 6E2C）；低优先。
...
- ⬜ **新参考研究（2026-08-25 加入，低优先）**：用户提供三份新参考已复制到 `reference/`
  ——`This.exe`（降群法程序，Thistlethwaite 群体降群流派）、`chichu v2011.xls` 与
  `一九四三阶魔方盲拧彳亍法公式(2).xls`（彳亍法盲拧公式，两份同源重叠多）。子代理研究中
  （后台 9029ac35）；结论回填暂存于此（降群法与「高级方法降群 ZZ ⬜」方向相关、彳亍公式
  表的规模/结构/可复用点）。结论暂存，不接入主干实现。
