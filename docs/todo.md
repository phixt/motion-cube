...
- ✅ **动作刻度多选批量改时间**：Ctrl+点击切换多选、Shift+点击区间多选、
  ...
  等比例重映射关键帧/接触轨道）
- ⬜ **复杂批量编辑（之后搞）**：动作/姿态批量——参考 RPE 的粉/蓝批量控制点
  （拖动批量加减时间/坐标、批量缩放）与方向键批量移动
- ⬜ **RPE（phiedit）深入调研——最低优先级**：安装包在
  D:\BaiduNetdiskDownload\rpe（1.2.1~1.6.0 + 说明书 PDF）；1.7.0 声称有
  AE 式频谱显示、拖拽直接编辑事件等，但**未开源，需逆向，很麻烦**——暂缓
- 已参考：多选方式（Ctrl+点击、Shift 框选/区间、Esc 取消）、批量改时间交互
...
3. 🔴 **自定义开始状态**（公式完成后非还原态；正放/倒放都要能自定义起始态；
   ...
   状态、倒放=还原态），自定义 UI 待做）
4. 🟡 **手指关节调整**（IP/PIP/MCP 角度输入，编辑器编辑能力核心）
