...
//
// 落地（2026-09-08 第四轮）：O 组 3 条已从 cuberoot-algs.json 数据层剔除（zbls 305→302），
// 原样留档 docs/archived/zbls-ogroup-3-cases.json。本脚本现在是**回归检查**：
// 期望输出分布 = {FR:302}、当前库内全槽已解形 = 0、归档内 = 3。
//
...
const arc = JSON.parse(readFileSync("docs/archived/zbls-ogroup-3-cases.json", "utf8")) as {
  cases: Array<{ name: string; subgroup: string; setup: string; id: number }>;
};
...
console.log("== cuberoot zbls（剔除后现状，期望 302 条）：槽位形状分布 ==");
for (const [k, v] of [...dist.entries()].sort()) console.log(`  ${k}: ${v}`);
console.log(`  解析失败: ${fail}`);

// ---- 归档的 3 条（全槽已解型，已从数据层剔除）----
console.log(`\n== 当前库内残留的全槽已解形 = ${extra.length}（期望 0）==`);
for (const e of extra) console.log(`  [${e.subgroup}] ${e.name}  id=${e.id}  setup=${e.setup}`);
console.log(`== 留档（docs/archived/zbls-ogroup-3-cases.json）内 = ${arc.cases.length} 条（期望 3）==`);
for (const e of arc.cases) console.log(`  [${e.subgroup}] ${e.name}  id=${e.id}  setup=${e.setup}`);
